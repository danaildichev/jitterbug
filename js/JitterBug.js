...
class JitterBug
{
    ...
    updateViewsCount()
    {
        ...
        setInterval(() =>
        {
            ...
            let nextViewCount = this.getRandomViewsCount();

            // randomly decide if views count should be increased or decreased
            let next = 0;
            if (Math.random() > 0.5) { next = this.viewBounds.start + nextViewCount; }
            else { next = this.viewBounds.start - nextViewCount; }

            // update view count displayed on the page
            for (const v of this.views) { v.innerText = next; }

            // reset the amount of time until next views count update
            viewsInterval = this.getRandomViewsInterval();
